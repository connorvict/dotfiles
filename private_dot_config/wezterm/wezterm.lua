...
-- Prefer a local theme module (themes/<name>.lua) if one exists,
-- otherwise fall back to a builtin wezterm color scheme.
local scheme
local localTheme
local ok, theme = pcall(require, "themes." .. G.colorscheme)
if ok and type(theme) == "table" and theme.colors then
	scheme = theme.colors()
	localTheme = theme
else
	scheme = wezterm.color.get_builtin_schemes()[G.colorscheme]
end
...
if localTheme and localTheme.window_frame then
	for property, value in pairs(localTheme.window_frame()) do
		config.window_frame[property] = value
	end
end
